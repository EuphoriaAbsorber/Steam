...
Отправка сообщения другим пользователям:
Нет необходимости писать большие сообщения, так как Steam не лучшие способ конкретно общения, так что одно сообщение можно оценить в среднем в 20 символов. Итого 20 КБ на одно сообщение. 
Ежедневный трафик в секунду: 70 000 000 * 20 * 10 / (24 * 60 * 60) = 162 037 КБ/с ~ 0.0001 ТБ/с.

Итоговый пиковый график 19.18 ТБ/с.

### 3. Логическая схема
![image](https://github.com/EuphoriaAbsorber/Steam/assets/65418582/ff9df025-2077-48e0-9c7e-3ba6625abe05)
...
Индексы:
  ...
  - поле User_id таблицы Game_on_account
  - поле Game_id таблицы Games
![физ схема4](https://github.com/EuphoriaAbsorber/Steam/assets/65418582/d9b4b51f-bfcf-49d0-8777-a7ce3e838912)
...
### 6. Схема проекта
![1](https://github.com/EuphoriaAbsorber/Steam/assets/65418582/b06c1a58-d0e1-4b45-bdf2-fc8d27374612)

### 7. Список серверов
### Хранилище

Рассмотрим FTPS-сервера, которые раздают игровые файлы с HDD дисков.
...
2. Остальные backend-сервера. На одном сервере будет основная PostgreSQL, ещё 3 аналогичных сервера для репликации. С двух серверов Slaves будет производиться чтение, на Master - запись, и один для бекапа. В [разделе 2](https://github.com/EuphoriaAbsorber/Steam/blob/main/README.md#rps) пиковый rps был оценен в ~ 24000, с чем эти сервера справятся.

Сборка сервера:

|RAM|CPU|Network|Disk|
|------|------------------------------|------|----------|
|8x64GB|2x6258R                       |10GB/s|10xSSD1T  |
